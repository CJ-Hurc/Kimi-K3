#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# Goal     : Fail-closed root entry for CJ-Hurc/Kimi-K3.
# Purpose  : Refuse direct execution unless KIMI_K3_ALLOWED=yes; then
...
# -----------------------------------------------------------------------------
if [[ "${KIMI_K3_ALLOWED:-}" != "yes" ]]; then
	echo "This script should not be run directly."
	exit 1
fi
ROOT="$(cd "$(dirname "$0")" && pwd)"
exec bash "$ROOT/devtools/kimi-k3/run.sh" "$@"

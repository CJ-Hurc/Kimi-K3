#!/usr/bin/env python3
"""Identity-bound universal repository occupancy for CJ-Hurc/Kimi-K3."""
...
def prove(scenario: str) -> dict[str, object]:
	stack = ROOT / "devtools" / "kimi-k3" / "run.sh"
	legacy = ROOT / "run.sh"
	if scenario == "startup":
		...
		return {
			"ok": ok,
			"evidence_refs": ["devtools/kimi-k3/run.sh", "startup-help"],
		}
	if scenario == "build":
		checks = [
		...
		ok = ok and (ROOT / "LICENSE").is_file()
		ok = ok and (ROOT / "assets" / "kimi-logo.png").is_file()
		ok = ok and (ROOT / "k3_tech_report.pdf").is_file()
	...
	if scenario == "cleanup":
		with tempfile.TemporaryDirectory(prefix="kimi-k3-ce2e-cleanup-") as scratch:
	...
	if scenario == "missing-env":
		proc = _run(["bash", str(legacy)], env={"KIMI_K3_ALLOWED": "no"})
		text = f"{proc.stdout}\n{proc.stderr}".lower()
		ok = proc.returncode != 0 and ("should not be run directly" in text or "kimi_k3_allowed" in text)
		return {
			"ok": ok,
			"evidence_refs": ["run.sh", "kimi-k3-allowed-fail-closed"],
		}
	if scenario == "invalid-config":
		...
		return {
			"ok": ok,
			"evidence_refs": ["devtools/kimi-k3/run.sh", "invalid-command-fail-closed"],
		}
...
def main() -> int:
	...
	for row in rows:
		...
		doc = {
			...
			"pack": "universal",
			"scenario": scenario,
			"prover": "kimi-k3-repository-occupancy",
